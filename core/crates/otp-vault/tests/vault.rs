//! Integration tests for otp-vault: round-trips, error mapping, tombstone
//! purge, v1 import, and a committed-fixture format-stability guard.

use aes_gcm::aead::{Aead, KeyInit};
use aes_gcm::{Aes256Gcm, Nonce};
use otp_core::{HashAlgorithm, OtpAccount, OtpType};
use otp_vault::{import_backup_v1, Vault, VaultError};
...
fn atomic_file_round_trip() {
    ...
    let (mut v, vmk) = new_vault_with_account();
    let bytes = v.to_bytes(NOW).expect("to_bytes");
    otp_vault::write_vault_file(&path, &bytes).expect("write");
    let read = otp_vault::read_vault_file(&path).expect("read");
