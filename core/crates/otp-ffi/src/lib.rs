//! otp-ffi: UniFFI facade exposed to Swift (apple/) and C# (windows/).
//! See docs/ARCHITECTURE.md §7 (frozen contract). Namespace: `otp`.
//!
//! All FFI types are defined here and converted from otp-core/otp-vault types.
//! This crate is a thin adapter layer: it owns the FFI record/enum definitions,
//! maps Core/Vault/Sync errors onto `OtpError`, and drives `otp_vault::Vault` +
//! `otp_sync::SyncEngine`. The exported SURFACE is frozen.
#![allow(unused_variables, dead_code)]

use std::path::Path;
use std::sync::{Arc, Mutex, MutexGuard};

uniffi::setup_scaffolding!("otp");
...
impl From<otp_core::CoreError> for OtpError {
    fn from(e: otp_core::CoreError) -> Self {
        match e {
            otp_core::CoreError::InvalidSecret(msg) => OtpError::InvalidSecret { msg },
            otp_core::CoreError::InvalidUri(msg) => OtpError::InvalidUri { msg },
            otp_core::CoreError::InvalidParameter(msg) => OtpError::InvalidParameter { msg },
        }
    }
}

impl From<otp_vault::VaultError> for OtpError {
    fn from(e: otp_vault::VaultError) -> Self {
        match e {
            otp_vault::VaultError::WrongPassword => OtpError::WrongPassword,
            otp_vault::VaultError::WrongKey => OtpError::WrongKey,
            otp_vault::VaultError::Corrupt(msg) => OtpError::Corrupt { msg },
            otp_vault::VaultError::Io(msg) => OtpError::Io { msg },
            otp_vault::VaultError::Crypto(msg) => OtpError::Crypto { msg },
            otp_vault::VaultError::Core(c) => c.into(),
        }
    }
}

impl From<otp_sync::SyncError> for OtpError {
    fn from(e: otp_sync::SyncError) -> Self {
        match e {
            otp_sync::SyncError::NotConfigured => OtpError::NotConfigured,
            otp_sync::SyncError::Network(msg) => OtpError::Network { msg },
            otp_sync::SyncError::Auth(msg) => OtpError::Auth { msg },
            otp_sync::SyncError::Conflict => OtpError::Conflict,
            otp_sync::SyncError::Backend(msg) => OtpError::Backend { msg },
            otp_sync::SyncError::Vault(v) => v.into(),
        }
    }
}

// ---------------------------------------------------------------------------
// Type conversions (FFI <-> otp-core)
// ---------------------------------------------------------------------------

impl From<OtpType> for otp_core::OtpType {
    fn from(t: OtpType) -> Self {
        match t {
            OtpType::Totp => otp_core::OtpType::Totp,
            OtpType::Hotp => otp_core::OtpType::Hotp,
        }
    }
}

impl From<otp_core::OtpType> for OtpType {
    fn from(t: otp_core::OtpType) -> Self {
        match t {
            otp_core::OtpType::Totp => OtpType::Totp,
            otp_core::OtpType::Hotp => OtpType::Hotp,
        }
    }
}

impl From<HashAlgorithm> for otp_core::HashAlgorithm {
    fn from(a: HashAlgorithm) -> Self {
        match a {
            HashAlgorithm::Sha1 => otp_core::HashAlgorithm::Sha1,
            HashAlgorithm::Sha256 => otp_core::HashAlgorithm::Sha256,
            HashAlgorithm::Sha512 => otp_core::HashAlgorithm::Sha512,
        }
    }
}

impl From<otp_core::HashAlgorithm> for HashAlgorithm {
    fn from(a: otp_core::HashAlgorithm) -> Self {
        match a {
            otp_core::HashAlgorithm::Sha1 => HashAlgorithm::Sha1,
            otp_core::HashAlgorithm::Sha256 => HashAlgorithm::Sha256,
            otp_core::HashAlgorithm::Sha512 => HashAlgorithm::Sha512,
        }
    }
}

impl From<OtpAccount> for otp_core::OtpAccount {
...
impl From<otp_core::OtpAccount> for OtpAccount {
    fn from(a: otp_core::OtpAccount) -> Self {
        OtpAccount {
            id: a.id,
            otp_type: a.otp_type.into(),
            secret: a.secret,
...
impl From<OtpFolder> for otp_core::OtpFolder {
...
impl From<otp_core::OtpFolder> for OtpFolder {
    fn from(f: otp_core::OtpFolder) -> Self {
        OtpFolder {
            id: f.id,
            name: f.name,
            icon: f.icon,
...
impl From<otp_core::OtpCode> for OtpCode {
    fn from(c: otp_core::OtpCode) -> Self {
        OtpCode {
            code: c.code,
...
/// Adapts a foreign (FFI) `SyncBackend` into the `otp_sync::SyncBackend` the
/// engine consumes, translating `OtpError` back into `SyncError`.
struct BackendAdapter {
    inner: Arc<dyn SyncBackend>,
}

fn otp_error_to_sync(e: OtpError) -> otp_sync::SyncError {
    match e {
        OtpError::NotConfigured => otp_sync::SyncError::NotConfigured,
        OtpError::Network { msg } => otp_sync::SyncError::Network(msg),
        OtpError::Auth { msg } => otp_sync::SyncError::Auth(msg),
        OtpError::Conflict => otp_sync::SyncError::Conflict,
        OtpError::Backend { msg } => otp_sync::SyncError::Backend(msg),
        OtpError::WrongKey => otp_sync::SyncError::Vault(otp_vault::VaultError::WrongKey),
        OtpError::WrongPassword => otp_sync::SyncError::Vault(otp_vault::VaultError::WrongPassword),
        OtpError::Corrupt { msg } => {
            otp_sync::SyncError::Vault(otp_vault::VaultError::Corrupt(msg))
        }
        OtpError::Io { msg } => otp_sync::SyncError::Vault(otp_vault::VaultError::Io(msg)),
        OtpError::Crypto { msg } => otp_sync::SyncError::Vault(otp_vault::VaultError::Crypto(msg)),
        other => otp_sync::SyncError::Backend(other.to_string()),
    }
}

impl otp_sync::SyncBackend for BackendAdapter {
    fn fetch(&self) -> Result<Option<otp_sync::RemoteBlob>, otp_sync::SyncError> {
        match self.inner.fetch().map_err(otp_error_to_sync)? {
            Some(b) => Ok(Some(otp_sync::RemoteBlob {
                data: b.data,
                etag: b.etag,
            })),
            None => Ok(None),
    ...
    ) -> Result<String, otp_sync::SyncError> {
...
impl OtpClient {
    fn from_vault(vault: otp_vault::Vault, vault_path: String) -> Arc<Self> {
        Arc::new(OtpClient {
...
fn persist(state: &mut ClientState, now: i64) -> Result<(), OtpError> {
    ...
    otp_vault::write_vault_file(Path::new(&state.vault_path), &bytes)?;
    Ok(())
}
...
impl OtpClient {
    ...
    pub fn create(vault_path: String, master_password: String) -> Result<Arc<Self>, OtpError> {
        let vault = otp_vault::Vault::create(&master_password)?;
    ...
    pub fn open_with_key(vault_path: String, vmk: Vec<u8>) -> Result<Arc<Self>, OtpError> {
        let data = otp_vault::read_vault_file(Path::new(&vault_path))?;
        let vault = otp_vault::Vault::open_with_key(&data, &vmk)?;
    ...
    pub fn open_with_password(
        ...
        let data = otp_vault::read_vault_file(Path::new(&vault_path))?;
        let vault = otp_vault::Vault::open_with_password(&data, &master_password)?;
        Ok(OtpClient::from_vault(vault, vault_path))
    }
    ...
    pub fn restore(
        ...
        let vault = otp_vault::Vault::open_with_password(&blob, &master_password)?;
        let client = OtpClient::from_vault(vault, vault_path);
        {
            let mut state = client.lock();
            persist(&mut state, now_ms())?;
    ...
    pub fn reload(&self) -> Result<(), OtpError> {
        ...
        let data = otp_vault::read_vault_file(Path::new(&state.vault_path))?;
        let vault = otp_vault::Vault::open_with_key(&data, &vmk)?;
        state.vault = vault;
        Ok(())
    }
    ...
    pub fn list_accounts(&self) -> Vec<OtpAccount> {
        let state = self.lock();
        let mut accounts: Vec<otp_core::OtpAccount> = state
            .vault
            .payload()
            .accounts
            .iter()
    ...
    pub fn add_account(&self, account: OtpAccount) -> Result<OtpAccount, OtpError> {
        let now = now_ms();
        let mut core: otp_core::OtpAccount = account.into();
        if core.id.trim().is_empty() {
            core.id = uuid::Uuid::new_v4().to_string();
        }
        core.secret = otp_core::normalize_secret(&core.secret)?;
    ...
    pub fn add_from_uri(&self, uri: String) -> Result<Vec<OtpAccount>, OtpError> {
        let now = now_ms();
        let parsed: Vec<otp_core::OtpAccount> = if uri.starts_with("otpauth-migration://") {
            otp_core::parse_migration_uri(&uri, now)?
        } else {
            vec![otp_core::parse_otpauth_uri(&uri, now)?]
        };
    ...
    pub fn list_folders(&self) -> Vec<OtpFolder> {
        let state = self.lock();
        let mut folders: Vec<otp_core::OtpFolder> = state
            .vault
            .payload()
            .folders
            .iter()
    ...
    pub fn add_folder(&self, folder: OtpFolder) -> Result<OtpFolder, OtpError> {
        let now = now_ms();
        let mut core: otp_core::OtpFolder = folder.into();
        if core.id.trim().is_empty() {
            core.id = uuid::Uuid::new_v4().to_string();
        }
        if core.created_at == 0 {
    ...
    pub fn code(&self, id: String, unix_time_ms: i64) -> Result<OtpCode, OtpError> {
        ...
        let code = otp_core::generate_code(account, unix_time_ms)?;
        Ok(code.into())
    }
    ...
    pub fn next_hotp(&self, id: String) -> Result<OtpCode, OtpError> {
        ...
        if account.otp_type != otp_core::OtpType::Hotp {
        ...
        let code = otp_core::generate_code(account, now)?;
    ...
    pub fn codes_at(&self, unix_time_ms: i64) -> Vec<AccountCode> {
        ...
        let mut accounts: Vec<otp_core::OtpAccount> = state
            ...
            .accounts
            .iter()
            .filter(|a| a.deleted_at.is_none() && a.otp_type == otp_core::OtpType::Totp)
            .cloned()
            .collect();
        sort_accounts(&mut accounts);
        accounts
            .into_iter()
            .filter_map(|a| {
                otp_core::generate_code(&a, unix_time_ms)
                    .ok()
    ...
    pub fn export_backup(&self, password: String) -> Result<Vec<u8>, OtpError> {
        ...
        let mut container = otp_vault::Vault::create(&password)?;
    ...
    pub fn import_backup(
        ...
        let imported = otp_vault::Vault::open_with_password(&data, &password)?;
    ...
    pub fn import_backup_v1(
        ...
        let now = now_ms();
        let payload = otp_vault::import_backup_v1(&data, &password, now)?;
        self.apply_import(payload, merge)
    }
    ...
    pub fn sync(&self, unix_time_ms: i64) -> Result<SyncOutcome, OtpError> {
        ...
        let outcome = otp_sync::SyncEngine::sync(&mut state.vault, &adapter, unix_time_ms)?;
...
impl OtpClient {
    ...
    fn apply_import(&self, payload: otp_vault::VaultPayload, merge: bool) -> Result<u32, OtpError> {
        let now = now_ms();
        let mut state = self.lock();
        let count: u32;
        if !merge {
...
fn sort_accounts(accounts: &mut [otp_core::OtpAccount]) {
...
pub fn parse_otpauth_uri(uri: String, now_ms: i64) -> Result<OtpAccount, OtpError> {
    let account = otp_core::parse_otpauth_uri(&uri, now_ms)?;
    Ok(account.into())
}

#[uniffi::export]
pub fn account_to_uri(account: OtpAccount) -> String {
    let core: otp_core::OtpAccount = account.into();
    otp_core::to_otpauth_uri(&core)
}

#[uniffi::export]
pub fn validate_secret(secret: String) -> bool {
    otp_core::validate_secret(&secret)
}
...
pub fn generate_totp_now(
    ...
    let code = otp_core::generate_totp(&secret, algorithm.into(), digits, period, unix_time_secs)?;
    Ok(code)
}
