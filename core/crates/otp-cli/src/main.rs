//! `otp` — CLI for the OTP Authenticator vault.
//! See docs/ARCHITECTURE.md §8 (frozen contract).
//!
//! Depends only on otp-core / otp-vault / otp-sync (never otp-ffi).
...
use otp_vault::{Vault, VaultPayload};
use serde::{Deserialize, Serialize};

const KEYRING_SERVICE: &str = "otp-auth";
const KEYRING_WEBDAV: &str = "otp-auth-webdav";
...
#[derive(Parser)]
#[command(name = "otp", version, about = "OTP Authenticator CLI")]
struct Cli {
    /// Path to the vault file (overridden by $OTP_VAULT; default: XDG data dir).
...
fn resolve_vault_path(flag: &Option<String>) -> Result<PathBuf> {
    if let Ok(p) = std::env::var("OTP_VAULT") {
        if !p.is_empty() {
...
fn env_password() -> Option<String> {
    std::env::var("OTP_VAULT_PASSWORD")
        .ok()
        .filter(|s| !s.is_empty())
}
...
/// $OTP_BACKUP_PASSWORD → $OTP_VAULT_PASSWORD → prompt.
fn env_backup_password() -> Option<String> {
    std::env::var("OTP_BACKUP_PASSWORD")
        .ok()
        .filter(|s| !s.is_empty())
}
...
/// Password for opening an existing vault: $OTP_VAULT_PASSWORD or a prompt.
...
impl Session {
    fn save(&mut self) -> Result<()> {
        ...
        otp_vault::write_vault_file(&self.path, &bytes)?;
        Ok(())
    }
}
...
fn open_session(flag: &Option<String>) -> Result<Session> {
    ...
    let data = otp_vault::read_vault_file(&path)?;
...
fn cmd_init(flag: &Option<String>) -> Result<()> {
    ...
    otp_vault::write_vault_file(&path, &bytes)?;
    keyring_store_vmk(&path, &vault.vmk());
...
fn cmd_add(flag: &Option<String>, args: AddArgs) -> Result<()> {
    ...
    let secret = otp_core::normalize_secret(&secret_raw)?;
...
fn cmd_code(flag: &Option<String>, args: CodeArgs) -> Result<()> {
    ...
    let code = otp_core::generate_code(account, now_ms())?;
    if args.copy {
...
fn cmd_uri(flag: &Option<String>, query: &str) -> Result<()> {
    ...
    println!("{}", otp_core::to_otpauth_uri(&accounts[idx]));
    Ok(())
}
...
fn cmd_qr(flag: &Option<String>, query: &str) -> Result<()> {
    ...
    let accounts = live_accounts_sorted(session.vault.payload());
    let idx = resolve_query(&accounts, query)?;
    let uri = otp_core::to_otpauth_uri(&accounts[idx]);
...
fn cmd_import(flag: &Option<String>, file: &str, merge: bool, legacy: bool) -> Result<()> {
    ...
    let payload = if legacy {
        otp_vault::import_backup_v1(&data, &pw, now_ms())?
    } else {
...
fn cmd_sync(flag: &Option<String>, action: SyncCmd) -> Result<()> {
    match action {
        ...
        SyncCmd::Now => {
            ...
            let mut session = open_session(flag)?;
            let outcome =
                otp_sync::SyncEngine::sync(&mut session.vault, backend.as_ref(), now_ms())?;
            session.save()?;
            println!(
...
fn build_backend(sc: &SyncConfig) -> Result<Box<dyn otp_sync::SyncBackend>> {
    match sc.backend.as_str() {
        "webdav" => {
            let pw = webdav_password(&sc.url)?;
            Ok(Box::new(otp_sync::webdav::WebDavBackend::new(
                sc.url.clone(),
                sc.user.clone(),
                pw,
            )))
...
fn cmd_restore(flag: &Option<String>, target: &str) -> Result<()> {
    ...
    let blob = if target.starts_with("http://") || target.starts_with("https://") {
        ...
        let backend = otp_sync::webdav::WebDavBackend::new(target.to_string(), user, pw);
        let remote = otp_sync::SyncBackend::fetch(&backend)?
...
fn cmd_passwd(flag: &Option<String>) -> Result<()> {
    ...
    let new = if let Ok(p) = std::env::var("OTP_NEW_PASSWORD") {
        if p.is_empty() {
            bail!("OTP_NEW_PASSWORD is empty");
        }
        p
    } else {
